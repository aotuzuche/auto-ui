...
> {
  ...
  public HHId: string
  public MMId: string
  public realScrollRowHeight: number
  public iScrollDay: any
  public iScrollDayY: any
  public iScrollHH: any
  public iScrollHHY: any
  public iScrollMM: any
  public iScrollMMY: any
  public constructor(props: TimePickerProps) {
    ...
    // 每一行的高度（即li的高度）
    this.realScrollRowHeight = (window as any).rem
      ? ((window as any).rem / 100) * 70
      : 70
  ...
  public componentDidUpdate(props: TimePickerProps) {
  ...
  public initScrolls = () => {
    ...
    if (!this.iScrollDay && this.dayId) {
      ...
      if (this.state.dayIndex !== 0) {
        this.iScrollDay.scrollTo(
          0,
          -this.realScrollRowHeight * this.state.dayIndex,
          0
        )
      }
      // 事件监听
    ...
    if (!this.iScrollHH && this.HHId) {
      ...
      if (this.state.HHIndex !== 0) {
        this.iScrollHH.scrollTo(
          0,
          -this.realScrollRowHeight * this.state.HHIndex,
          0
        )
      }
      // 事件监听
    ...
    if (!this.iScrollMM && this.MMId) {
      ...
      if (this.state.MMIndex !== 0) {
        this.iScrollMM.scrollTo(
          0,
          -this.realScrollRowHeight * this.state.MMIndex,
          0
        )
      }
      // 事件监听
  ...
  // 滚动结束后的计算
  public scrollEndCalc = (scroll: any, type: string) => {
    if (this['needCalc' + type]) {
      ...
      // 如果是日期滚动，更新小时和分钟数据
      if (type === 'Day') {
        const times = event.transTimeList(
          event.getTimeList(this.props.data[row])
        )
        let hIndex = [times.HH as any[]].indexOf(this.state.HHValue)
        let hChange = hIndex === -1
        hIndex = hIndex > -1 ? hIndex : 0

        let mIndex = (times[times.HH[hIndex]] as any[]).indexOf(
          this.state.MMValue
        )
      ...
      // 分钟如果有相同的值，不动，不然则恢复到默认第一个值
        ...
        let mIndex = (mList as any).indexOf(this.state.MMValue)
        mIndex = mIndex > -1 ? mIndex : 0
        this.setState(
          {
            HHIndex: row,
  ...
  public renderDayList() {
    return (
      <div className="x-time-picker__day-list" id={this.dayId}>
        <ul>
          {this.state.days.map((i: ReactNode, index: number) => {
  ...
  public renderHHList() {
    ...
    return (
      <div className="x-time-picker__hh-list" id={this.HHId}>
        <ul>
          {this.state.HH.map((i: any, index: number) => {
            let val = i + '点'
            if (/(h+)/.test(hFormat)) {
  ...
  public renderMMList() {
    ...
    return (
      <div className="x-time-picker__mm-list" id={this.MMId}>
        <ul>
          {this.state.MM.map((i: any, index: number) => {
            let val = i + '分'
            if (/(m+)/.test(mFormat)) {
