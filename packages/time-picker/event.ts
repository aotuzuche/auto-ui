...
class Event {
  ...
  public MM: any;
  // 获取时间列表，并format格式
  public getDayList(o: any, format = ["MM月dd日 周wk", "h点", "m分"]) {
    const days: any = [];
  ...
  // 根据时间转换出state需要的数据
  public getStateByTime(t: any, data: any) {
    let defDay: any;
    let defHH: any;
    let defMM: any;
    ...
    if (hasMatchDay && defMM) {
      for (let i = 0; i < (times[times.HH[HHIndex]] as []).length; i++) {
  ...
  public transTimeList(times: any) {
    const trans = {
      HH: []
    };
  ...
  // 根据两个范围获取可用的时间
  public getTimeRange(start: any, end: any) {
    // 判断格式是否正确
  ...
  public getTimeList = (day: any) => {
    ...
    return times.sort((a: any, b: any) => a - b);
  };
}
